...
const url = "https://excellanto.com/website-development/";
...
const out = {
  ...
  paragraphs: [...t.matchAll(/<p[^>]*>([\s\S]*?)<\/p>/gi)]
    .map((m) => clean(m[1]))
    .filter((p) => p.length > 15 && !p.startsWith("Excellanto is at the forefront")),
...
writeFileSync("scripts/website-dev-ref.json", JSON.stringify(out, null, 2));
